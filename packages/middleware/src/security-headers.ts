...
async function handleCorsPreflightRequest(
  c: any,
...
async function applySecurityHeaders(c: any, config: SecurityConfig): Promise<void> {
  ...
  if (config.xssProtection) {
    const value = typeof config.xssProtection === "string" ? config.xssProtection : "1; mode=block";
...
async function applyResponseHeaders(c: any, config: SecurityConfig): Promise<void> {
  ...
  if (config.hsts && isHttps) {
    ...
    let hstsValue = `max-age=${hstsConfig.maxAge || 31_536_000}`;
