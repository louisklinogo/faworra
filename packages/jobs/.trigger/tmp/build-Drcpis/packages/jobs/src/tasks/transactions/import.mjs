...
var require_papaparse = __commonJS({
  "../../node_modules/.bun/papaparse@5.5.3/node_modules/papaparse/papaparse.js"(exports, module) {
    init_esm();
...
// src/tasks/transactions/import.ts
init_esm();
...
var importSchema = external_exports.object({
  ...
  currency: external_exports.string().min(3).max(3),
  filePath: external_exports.array(external_exports.string()),
  inverted: external_exports.boolean().optional().default(false),
  mappings: external_exports.object({ amount: external_exports.string(), date: external_exports.string(), description: external_exports.string() })
});
var importTransactions = schemaTask({
  id: "import-transactions",
  schema: importSchema,
  maxDuration: 120,
  queue: { concurrencyLimit: 5 },
  run: /* @__PURE__ */ __name(async ({ teamId, bankAccountId, currency, filePath, mappings, inverted }) => {
    ...
    const upsertBatch = /* @__PURE__ */ __name(async (rows) => {
      if (!rows.length) return;
      const { error: error2 } = await supabase.from("transactions").upsert(rows, { onConflict: "internal_id", ignoreDuplicates: true }).select("id");
    ...
    const toDateOnly = /* @__PURE__ */ __name((s) => {
      try {
        const d = new Date(s);
    ...
    await new Promise((resolve, reject) => {
      import_papaparse.default.parse(content, {
        header: true,
        skipEmptyLines: true,
        worker: false,
        chunk: /* @__PURE__ */ __name(async (chunk, parser) => {
          parser.pause();
          const now = Date.now();
        ...
        complete: /* @__PURE__ */ __name(() => resolve(), "complete"),
        error: /* @__PURE__ */ __name((e) => reject(e), "error")
      });
    });
  }, "run")
});
