...
import { communicationThreadTags, tags } from "../schema";
...
export async function getTagsWithUsage(db: DbClient, params: { teamId: string }) {
  const rows = await db
    .select({
      id: tags.id,
      name: tags.name,
      color: tags.color,
      createdAt: tags.createdAt,
      usageCount: sql<number>`count(${communicationThreadTags.id})`.as("usage_count"),
    })
    .from(tags)
    .leftJoin(
      communicationThreadTags,
      and(
        eq(communicationThreadTags.tagId, tags.id),
        eq(communicationThreadTags.teamId, tags.teamId),
      ),
    )
    .where(eq(tags.teamId, params.teamId))
    .groupBy(tags.id)
    .orderBy(tags.name);
  return rows.map((r) => ({ ...r, usageCount: Number((r as any).usageCount ?? 0) }));
}
