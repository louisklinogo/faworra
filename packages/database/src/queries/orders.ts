...
export async function getOrdersWithClients(
  ...
  const { teamId, limit = 50, cursor, status, search } = params;

  const searchClause = search
    ? or(
        sql`${orders.searchTsv} @@ plainto_tsquery('simple', ${search})`,
        sql`${clients.searchTsv} @@ plainto_tsquery('simple', ${search})`,
      )
    : sql`true`;
  ...
  const baseWhere = and(
    ...
    status ? eq(orders.status, status as any) : sql`true`,
    searchClause,
  );
