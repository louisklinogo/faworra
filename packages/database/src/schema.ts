...
// Custom Postgres type for tsvector
...
// Orders enum (align with live DB)
...
// Team role enum (align with live DB)
export const teamRoleEnum = pgEnum("team_role", ["owner", "admin", "agent", "viewer"]);
...
export const clients = pgTable(
  "clients",
  {
    ...
    // Index created via manual migration using GIN
...
export const orders = pgTable(
  "orders",
  {
    ...
    // Align status to order_status enum
    ...
    deletedAt: timestamp("deleted_at", { withTimezone: true }),
    // Full-text search vector for fast search (order number, notes)
    searchTsv: tsvector("search_tsv"),
  },
  (table) => ({
...
export const invoices = pgTable(
  ...
  (table) => ({
    ...
    scheduledSendAtIdx: index("idx_invoices_scheduled_send_at").on(table.scheduledSendAt),
    lastRemindedAtIdx: index("idx_invoices_last_reminded_at").on(table.lastRemindedAt),
    tokenIdx: index("idx_invoices_token").on(table.token),
  }),
);

// Team-scoped invoice templates (Midday parity)
export const invoiceTemplates = pgTable("invoice_templates", {
  teamId: uuid("team_id")
    .primaryKey()
    .references(() => teams.id, { onDelete: "cascade" }),
  template: jsonb("template").default(sql`'{}'::jsonb`).notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
});
...
export const products = pgTable(
  "products",
  {
    ...
    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
    deletedAt: timestamp("deleted_at", { withTimezone: true }),
    // Full-text search vector for fast search (name, slug, description)
    searchTsv: tsvector("search_tsv"),
  },
  (table) => ({
...
// Saved inbox views per team
export const savedInboxViews = pgTable(
  "saved_inbox_views",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    teamId: uuid("team_id")
      ...
      .references(() => teams.id, { onDelete: "cascade" }),
    name: text("name").notNull(),
    filter: jsonb("filter").default(sql`'{}'::jsonb`).notNull(),
    ownerUserId: uuid("owner_user_id").references(() => users.id, { onDelete: "set null" }),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => ({
    teamIdx: index("idx_saved_views_team").on(table.teamId),
  }),
);
...
export const assignmentPolicies = pgTable(
  "assignment_policies",
  {
    ...
    teamId: uuid("team_id")
      .notNull()
      .references(() => teams.id, { onDelete: "cascade" }),
    policy: text("policy").notNull(), // round_robin | capacity
    limits: jsonb("limits").default(sql`'{}'::jsonb`).notNull(),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => ({
    teamIdx: index("idx_assignment_policies_team").on(table.teamId),
  }),
);
...
export const macros = pgTable(
  ...
  (table) => ({
    teamIdx: index("idx_macros_team").on(table.teamId),
  }),
);
// Tags assigned to communication threads (many-to-many)
export const communicationThreadTags = pgTable(
  "communication_thread_tags",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    teamId: uuid("team_id")
      .notNull()
  ...
  (table) => ({
    teamIdx: index("idx_comm_thread_tags_team").on(table.teamId),
    threadIdx: index("idx_comm_thread_tags_thread").on(table.threadId),
    tagIdx: index("idx_comm_thread_tags_tag").on(table.tagId),
    uniquePerThreadTag: uniqueIndex("uq_comm_thread_tag").on(table.threadId, table.tagId),
  }),
);
...
// User Invites - pending invitations to join a team
export const userInvites = pgTable(
  "user_invites",
  {
    ...
    role: teamRoleEnum("role").notNull(),
    // Optional join code; generated at application layer
    code: text("code"),
    invitedBy: uuid("invited_by").references(() => users.id, { onDelete: "cascade" }),
  },
  (table) => ({
    teamIdx: index("user_invites_team_id_idx").on(table.teamId),
    uniqueTeamEmail: index("uq_user_invites_team_email").on(table.teamId, table.email),
    uniqueCode: index("uq_user_invites_code").on(table.code),
  }),
);
...
export const transactions = pgTable(
  "transactions",
  {
    ...
    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
    deletedAt: timestamp("deleted_at", { withTimezone: true }),
    // Full-text search vector (align with live DB column name)
  ...
  (table) => ({
    ...
    internalIdUnique: uniqueIndex("uq_transactions_internal_id").on(table.internalId),
...
// =========================================================================
// Midday parity: Exchange rates and embeddings
// =========================================================================
...
export const documentTagEmbeddings = pgTable(
  ...
  (table) => ({
    // Exact HNSW index is created via manual migration for pgvector
    // Placeholder simple index name for Drizzle awareness
    documentTagEmbeddingsIdx: index("document_tag_embeddings_idx").on(table.slug),
  }),
);
...
export const transactionCategoryEmbeddings = pgTable(
  ...
  (table) => ({
    // Exact HNSW vector index created via manual migration
    transactionCategoryEmbeddingsSystemIdx: index("transaction_category_embeddings_system_idx").on(
      table.system,
    ),
  }),
);
...
// Event outbox for reliable realtime
...
export const documents = pgTable(
  "documents",
  {
    ...
    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
    deletedAt: timestamp("deleted_at", { withTimezone: true }),
    // Full-text search vector (name + tags + path tokens)
    searchTsv: tsvector("search_tsv"),
  },
  (table) => ({
...
export const transactionsRelations = relations(transactions, ({ one, many }) => ({
  ...
  // Composite relation by (team_id, category_slug) -> (team_id, slug)
