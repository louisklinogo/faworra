...
export const jobStatusResponseSchema = z.object({
	id: z.string(),
	status: z.enum(["waiting", "active", "completed", "failed", "delayed", "prioritized", "unknown"]),
...
export async function triggerJob(
	...
	const jobId = options.jobId ?? createCompositeJobId(queueName, name, crypto.randomUUID());
...
export async function getJobStatus(
	...
	if (options.validateTeamId && job.data?.teamId) {
		if (job.data.teamId !== options.validateTeamId) {
