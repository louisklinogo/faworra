...
export async function GET(request: NextRequest) {
  ...
  try {
    if (code) {
      ...
      if (user) {
        ...
        try {
          ...
          logger.error({ err: bootstrapError, userId: user.id }, "auth user bootstrap error");
        }
        ...
        try {
          ...
          logger.error({ err: error, userId: user.id }, "auth team check error");
          await trackAuthEvent({
            event: "SignIn.Failed",
    ...
    logger.error({ err: error, provider }, "unexpected auth callback error");
