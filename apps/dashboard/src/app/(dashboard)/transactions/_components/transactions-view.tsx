...
export function TransactionsView({
  ...
  const enrichedInput = useMemo<EnrichedListInput>(() => {
    ...
    const input: EnrichedListInput = {
      ...
      status: statusesSorted?.length ? (statusesSorted as EnrichedListInput["status"]) : undefined,
      categories: categoriesSorted?.length ? categoriesSorted : undefined,
      tags: tagsSorted?.length ? tagsSorted : undefined,
      accounts: accountsSorted?.length ? accountsSorted : undefined,
      assignees: assigneesSorted?.length ? assigneesSorted : undefined,
      isRecurring,
      search: q || undefined,
  ...
  const [selectedTrx, _setSelectedTrx] = useState<EnrichedItem | null>(null);
  ...
  const _applyParsedFilters = (p: Record<string, unknown>) => {
  ...
  const menuUpdate = trpc.transactions.bulkUpdate.useMutation({
    async onMutate(variables) {
      ...
      await utils.transactions.enrichedList.cancel(enrichedInput);
      const prev = utils.transactions.enrichedList.getInfiniteData(enrichedInput) as
        | {
      ...
      if (prev) {
        ...
        utils.transactions.enrichedList.setInfiniteData(enrichedInput, next as any);
      }
      return { prev };
    },
    onError: (_err, _vars, ctx) => {
      if (ctx?.prev) {
        utils.transactions.enrichedList.setInfiniteData(enrichedInput, ctx.prev as any);
      }
    },
    onSettled: async () => {
      await invalidateTransactions();
  ...
  const bulkDelete = trpc.transactions.bulkDelete.useMutation({
    async onMutate(variables) {
      ...
      await utils.transactions.enrichedList.cancel(enrichedInput);
      const previous = utils.transactions.enrichedList.getInfiniteData(enrichedInput) as
        | {
            pages: Array<{ items: EnrichedItem[]; nextCursor?: string | null }>;
            pageParams: unknown[];
          }
    ...
    onError: (_err, _vars, ctx) => {
      if (ctx?.previous) {
        utils.transactions.enrichedList.setInfiniteData(enrichedInput, ctx.previous as any);
      }
    },
    onSettled: async () => {
      await invalidateTransactions();
  ...
  }, [currencyCode, bulkDelete, toast, openParams, menuUpdate.mutateAsync]);
  ...
  return (
    <div className="flex flex-col gap-6">
      <ImportModal currencies={uniqueCurrencies} defaultCurrency={currency || "GHS"} />
      <TransactionSheet />
