...
import { taxTypes } from "@Faworra/utils/tax";
...
export function CategoryEditSheet({ open, onOpenChange, id, categories = [] }: Props) {
  ...
  const _flat = useMemo(() => flatten(categories), [categories]);
  const currentNode = useMemo(() => (id ? findNode(categories, id) : null), [categories, id]);
  const hasChildren = Boolean(currentNode?.children?.length && currentNode.children.length > 0);
      ...
      <SheetContent className="flex flex-col overflow-hidden p-0">
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          <div className="scrollbar-hide flex-1 space-y-4 overflow-y-auto px-6 py-4">
            ...
            <div className="space-y-1">
              <span className="text-muted-foreground text-xs">Parent Category (Optional)</span>
              {hasChildren ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <SelectTaxType
                  value={taxType ?? ""}
                  onChange={(v) => setTaxType(v || undefined)}
                />
              </div>
              <div className="space-y-1">
            ...
            <div className="text-muted-foreground text-xs">
              {taxTypes.find((t) => t.value === taxType)?.description ||
                "For unsupported or internal tax logic."}
            </div>

            <div className="space-y-1">
              <span className="text-muted-foreground text-xs">Report Code</span>
              <Input
