...
export function CategoryCreateSheet({
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      taxType: defaultTaxType ?? undefined,
      taxRate: "",
      taxReportingCode: "",
      excluded: false,
    },
  ...
  // When the sheet is opened for a new create, seed defaults without overriding user edits mid-session
  useEffect(() => {
    if (open) {
      form.reset({
        name: "",
          ...
          <form className="flex min-h-0 flex-1 flex-col" onSubmit={form.handleSubmit(onSubmit)}>
            <div className="scrollbar-hide flex-1 space-y-4 overflow-y-auto px-6 py-4">
              ...
              <div className="text-muted-foreground text-xs">
                {taxTypes.find((t) => t.value === form.watch("taxType"))?.description ||
