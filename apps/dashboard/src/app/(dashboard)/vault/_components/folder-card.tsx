...
export function FolderCard({ name, count, onClick }: FolderCardProps) {
  return (
    <button
      className="group relative flex h-72 cursor-pointer flex-col gap-3 border p-4 text-left text-muted-foreground transition-colors duration-200 hover:bg-accent"
      onClick={onClick}
      type="button"
    >
