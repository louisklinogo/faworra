...
export function OrdersView({ initialOrders = [] }: OrdersViewProps) {
  ...
  const [{ q: urlSearch, status: statusFilterRaw }, setFilters] = useQueryStates(
    {
  ...
  const urlStatusFilter = (statusFilterRaw as (typeof ORDER_STATUSES)[number]) || "all";
  ...
  const [pages, { fetchNextPage, hasNextPage, isFetchingNextPage }] =
    trpc.orders.list.useSuspenseInfiniteQuery(
      {
        limit: 50,
        status: urlStatusFilter === "all" ? undefined : (urlStatusFilter as any),
  ...
  const filteredOrders = orders; // server-side filtering applied via tRPC params
  ...
  const rowVirtualizer = useVirtualizer({
    ...
    estimateSize: () => 64,
    overscan: 8,
    enabled: filteredOrders.length > 50,
  });
  const virtualRows =
    filteredOrders.length > 50
      ? rowVirtualizer.getVirtualItems()
      : filteredOrders.map((_, index) => ({ index }) as any);
  const totalSize = rowVirtualizer.getTotalSize();
  const paddingTop = filteredOrders.length > 50 && virtualRows.length ? virtualRows[0]!.start : 0;
  const paddingBottom =
    filteredOrders.length > 50 && virtualRows.length
    ...
    <div className="flex flex-col gap-6">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {ORDER_STATUSES.map((status) => (
                  <DropdownMenuItem
                    key={status}
                    onClick={() => setFilters({ status: status === "all" ? null : status })}
                  >
                    <span className={urlStatusFilter === status ? "font-semibold" : ""}>
      ...
      {filteredOrders.length === 0 ? (
        <EmptyState
          ...
          title={urlSearch || urlStatusFilter !== "all" ? "No results" : "No orders"}
        />
      ) : (
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <div
              className="relative max-h-[calc(100vh-360px)] overflow-auto"
            ...
            >
              <Table>
                ...
                <TableBody>
                  ...
                  {virtualRows.map((vr: any) => {
                    const order = filteredOrders[vr.index];
