...
export const getCurrentTeamId = cache(async () => {
  ...
  return userRecord?.currentTeamId || null;
});
    ...
    links: [
      httpBatchLink({
        ...
        async headers() {
          ...
          // Derive locale from incoming request headers
          const h = await nextHeaders();
          const accept = h.get("accept-language") || undefined;
          const resolvedLocale = accept?.split(",")[0]?.trim();
          return {
            Authorization: session?.access_token ? `Bearer ${session.access_token}` : "",
            ...(resolvedLocale ? { "x-user-locale": resolvedLocale } : {}),
