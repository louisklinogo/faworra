...
const periodOptions: { id: Period; label: string; range: () => { start: string; end: string } }[] =
  [
    ...
    },
    {
      ...
      range: () => {
        const now = new Date();
        const start = new Date(now.getTime() - 89 * 86_400_000);
        return { start: start.toISOString().slice(0, 10), end: now.toISOString().slice(0, 10) };
      },
    },
    {
    ...
    <div className="relative aspect-square overflow-hidden border">
      <div className="flex-col p-4 md:p-8">
        <div className="flex justify-between">
          <div>
            <Link
              href={`/transactions?type=expense&start=${range.start}&end=${range.end}`}
              prefetch
            >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-[180px]">
              {periodOptions.map((opt) => (
                <DropdownMenuCheckboxItem
                  checked={opt.id === period}
                  key={opt.id}
                  onCheckedChange={() => setPeriod(opt.id)}
                >
                  {opt.label}
                </DropdownMenuCheckboxItem>
              ))}
        ...
        {byCategory.arr.length === 0 ? (
          <div className="flex aspect-square items-center justify-center">
            <p className="-mt-12 text-muted-foreground text-sm">No spending yet</p>
          </div>
        ) : (
          <ul className="scrollbar-hide mt-4 aspect-square space-y-2 overflow-auto pb-24">
            {byCategory.arr.slice(0, 12).map((row) => {
              const pct = Math.round((row.amount / byCategory.total) * 100);
              return (
                <li className="flex items-center gap-3" key={row.slug}>
                  <span
                    className="h-2 w-2 rounded-sm"
                    style={{ backgroundColor: row.color || "#9b9b9b" }}
                  />
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="h-2 rounded bg-muted">
                      <div
                        ...
                        style={{ width: `${pct}%`, backgroundColor: row.color || "#9b9b9b" }}
                      />
                    </div>
                  </div>
                </li>
