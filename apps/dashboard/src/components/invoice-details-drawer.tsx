      ...
      <DrawerContent>
        ...
        <div className="space-y-3 p-4">
          <div className="flex items-center gap-6 text-sm">
            <div>Total: {formatAmount({ currency, amount: invoice?.amount || 0 })}</div>
            <div>Paid: {formatAmount({ currency, amount: paid || 0 })}</div>
            <div>Outstanding: {formatAmount({ currency, amount: outstanding || 0 })}</div>
          </div>
              ...
              <div className="space-y-2">
                {allocations.map((a) => (
                  <div className="flex items-center justify-between text-sm" key={a.id}>
                    <div>
                      <div>
                        TX {a.transaction?.transaction_number} — {formatAmount({ currency, amount: Number(a.amount) || 0 })}
                      </div>
