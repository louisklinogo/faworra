...
export function FilterDropdown({ values, onChange, mode = "icon", hide }: Props) {
  ...
  useEffect(() => {
    ...
    setMinText(fmt(min));
    setMaxText(fmt(max));
  }, [values.amountMin, values.amountMax, minBound, maxBound, fmt]);
  ...
  const derivedHide = hide ?? {
    ...
    statuses: Boolean(values.statuses?.length),
    categories: Boolean(values.categories?.length),
    tags: Boolean(values.tags?.length),
    accounts: Boolean(values.accounts?.length),
    assignees: Boolean(values.assignees?.length),
