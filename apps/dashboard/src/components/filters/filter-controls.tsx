...
export function Control({ field, value, onChange }: CommonProps) {
  ...
  if (field.type === "multi") {
    ...
    return (
      <div className="w-[240px]">
        <ComboboxMulti
          items={items}
          onChange={(ids) => setLocal(ids?.length ? ids : undefined)}
  ...
  if (field.type === "date_range") {
    ...
    const from = local?.[fromKey] ? new Date(local[fromKey]) : undefined;
    const to = local?.[toKey] ? new Date(local[toKey]) : undefined;
