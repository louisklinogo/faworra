  ...
  const m = trpc.orders.create.useMutation({
    onSuccess: async () => {
      await Promise.all([
        ...
        utils.analytics.highestValueOrder.invalidate(),
        utils.analytics.completedOrdersThisMonth.invalidate(),
        utils.analytics.pendingOrdersCount.invalidate(),
        utils.analytics.averageOrderValue.invalidate(),
      ]);
