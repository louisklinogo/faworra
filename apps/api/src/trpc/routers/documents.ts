...
export const documentsRouter = createTRPCRouter({
  ...
  getById: teamProcedure.input(z.object({ id: z.string().uuid() })).query(
    async ({ ctx, input }) =>
      await getDocumentById(ctx.db, {
        id: input.id,
        teamId: ctx.teamId,
      }),
  ),
  ...
  create: teamProcedure.input(documentCreateSchema).mutation(
    async ({ ctx, input }) =>
      await createDocument(ctx.db, {
        ...input,
        teamId: ctx.teamId,
  ...
  update: teamProcedure.input(documentUpdateSchema).mutation(
    async ({ ctx, input }) =>
  ...
  delete: teamProcedure.input(z.object({ id: z.string().uuid() })).mutation(
    async ({ ctx, input }) =>
      await deleteDocument(ctx.db, {
        id: input.id,
        teamId: ctx.teamId,
      }),
  ),
  ...
  stats: teamProcedure.query(
    async ({ ctx }) =>
      await getDocumentStats(ctx.db, {
        teamId: ctx.teamId,
      }),
  ),

  // Get all tags with usage count
  tags: teamProcedure.query(
    async ({ ctx }) =>
      await getAllDocumentTags(ctx.db, {
        teamId: ctx.teamId,
      }),
  ),
});
