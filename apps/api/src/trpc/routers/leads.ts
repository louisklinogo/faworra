...
export const leadsRouter = createTRPCRouter({
  ...
  get: teamProcedure
    ...
    .query(
      async ({ ctx, input }) =>
        await getLead(ctx.db, { teamId: ctx.teamId!, leadId: input.leadId }),
    ),
  createFromThread: teamProcedure
    .input(
      z.object({
        threadId: z.string().uuid(),
        manualScore: z.number().int().min(SCORE_MIN).max(SCORE_MAX).optional(),
    ...
    .input(
      z.object({
        status: z
          ...
          .default("all"),
        minScore: z.number().int().min(SCORE_MIN).max(SCORE_MAX).optional(),
        limit: z.number().int().min(LIMIT_MIN).max(LIMIT_MAX).optional().default(LIMIT_DEFAULT),
        cursor: z
