...
export const clientsRouter = createTRPCRouter({
  ...
  byId: teamProcedure
    ...
    .query(async ({ ctx, input }) => await getClientById(ctx.db, input.id, ctx.teamId)),

  // Create a new client
  create: teamProcedure.input(clientInsertSchema).mutation(
    async ({ ctx, input }) =>
      await createClient(ctx.db, {
        ...input,
        teamId: ctx.teamId,
      }),
  ),
  ...
  update: teamProcedure.input(clientUpdateSchema).mutation(({ ctx, input }) => {
    const { id, ...data } = input;
  ...
  delete: teamProcedure
    .input(z.object({ id: z.string().uuid() }))
    .mutation(async ({ ctx, input }) => await deleteClient(ctx.db, input.id, ctx.teamId)),
});
