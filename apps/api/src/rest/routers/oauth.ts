...
import { z } from "zod";
import { ensure, getAccessTokenFromHeader, HTTP, parseScopes } from "../../lib/http";
...
app.openapi(
  ...
  async (c) => {
    ...
    function mapAuthCodeError(msg: string): HTTPException | null {
      const patterns: Array<{ k: string; message: string }> = [
        {
          k: "Authorization code expired",
          message: "The authorization code has expired. Please restart the OAuth flow.",
        },
        {
          k: "Authorization code already used",
          message:
            "The authorization code has already been used. All related tokens have been revoked for security.",
        },
        {
          k: "Invalid authorization code",
      ...
      for (const { k, message } of patterns) {
        if (msg.includes(k)) {
          return new HTTPException(HTTP.BAD_REQUEST, { message });
        }
      }
      return null;
    }

    async function handleAuthorizationCode(application: any) {
      ensure(
        code && redirect_uri,
      ...
      try {
        ...
        const mapped = mapAuthCodeError(msg);
        if (mapped) {
          throw mapped;
        }
        throw new HTTPException(HTTP.BAD_REQUEST, {
    ...
    function mapRefreshError(msg: string): HTTPException | null {
      if (msg.includes("Invalid refresh token")) {
        return new HTTPException(HTTP.BAD_REQUEST, { message: "Invalid refresh token" });
      }
      if (msg.includes("expired")) {
        return new HTTPException(HTTP.BAD_REQUEST, { message: "Refresh token expired" });
      }
      if (msg.includes("revoked")) {
        return new HTTPException(HTTP.BAD_REQUEST, { message: "Refresh token revoked" });
      }
      return null;
    }
    ...
    function handleRefreshToken() {
      ...
      try {
        const _requestedScopes = scope ? parseScopes(scope) : undefined;
        ...
        const mapped = mapRefreshError(em);
        if (mapped) {
          throw mapped;
        }
    ...
    try {
      ...
      if (grant_type === "authorization_code") {
        return handleAuthorizationCode(application);
      }
