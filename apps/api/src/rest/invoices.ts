...
export function registerInvoicesRoutes(app: Hono<ApiEnv>) {
  ...
  app.post("/invoices/:id/payments", handleCreateInvoicePayment);

  // Send invoice via WhatsApp (enqueue outbox)
  app.post("/invoices/:id/send-whatsapp", handleSendInvoiceWhatsApp);
}
...
async function handleCreateInvoicePayment(c: Context<ApiEnv>): Promise<Response> {
  try {
    ...
    const schema = z.object({
      ...
      currency: z.string().min(CURRENCY_CODE_MIN).max(CURRENCY_CODE_MAX).default("GHS"),
    ...
    if (!parsed.success) {
      return c.json({ error: "invalid payload" }, HTTP.BAD_REQUEST);
    }
    ...
    if (iErr || !inv) {
      return c.json({ error: "invoice not found" }, HTTP.NOT_FOUND);
    }
    ...
    if (tErr || !createdTrx) {
      return c.json(
        { error: tErr?.message || "failed to create transaction" },
    ...
    if (aErr) {
      return c.json({ error: aErr.message }, HTTP.INTERNAL_SERVER_ERROR);
    }
...
async function handleSendInvoiceWhatsApp(c: Context<ApiEnv>): Promise<Response> {
  try {
    const id = c.req.param("id");
    const supabase = c.get("supabaseAdmin");
    const teamId = c.get("teamId");
    ...
    if (!phone) {
      return c.json({ error: "client has no whatsapp" }, HTTP.BAD_REQUEST);
    }
    ...
    if (aErr || !acc) {
      return c.json({ error: "no whatsapp account for team" }, HTTP.BAD_REQUEST);
    }
    ...
    const msg =
      `Hi ${name || ""} — your order ${orderNo || ""} is ready. Total: ₵${inv.amount}. Due ${due}. Invoice: ${
        url || "[ask for PDF]"
      }`.trim();
