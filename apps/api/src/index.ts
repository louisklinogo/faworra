...
app.use(
  ...
  cors({
    origin: (origin) => {
      if (!origin) return "*";
      if (!apiCfg.corsOrigins.length) return origin; // permissive if none configured
      return apiCfg.corsOrigins.includes(origin) || apiCfg.corsOrigins.some((o) => origin.startsWith(o)) ? origin : "";
    },
...
export default {
  ...
  // Increase to avoid dev timeouts during heavy queries/uploads
  idleTimeout: 60,
  fetch: app.fetch,
};
