---
description: Local cheat sheet for Factory CLI custom slash commands
---

# Custom Slash Commands – Quick Reference

- Locations scanned:
  - Workspace: `<repo>/.factory/commands` (overrides personal)
  - Personal: `~/.factory/commands` (always scanned)
- Registered files: Markdown (`*.md`) and executables with a shebang (`#!`).
- Slugging: filename lowercased; spaces → `-`; non-URL chars dropped (`Code Review.mdx` → `/code-review`).
- Usage: `/command-name <args>`; open `/commands` to browse, reload (R), or import.
- Top-level only: nested folders inside `commands/` are ignored.
