...
async function run() {
  ...
  const filePath = join(import.meta.dir, "..", "drizzle", "manual-migrations", "0057_drop_currency_defaults.sql");
  const sql = readFileSync(filePath, "utf-8");

  const client = new Client({ connectionString });
  await client.connect();
  try {
    console.log("Applying migration 0057_drop_currency_defaults.sql ...");
